<?php namespace VaahCms\Modules\LeadManagement\Models;
...
use WebReinvent\VaahCms\Entities\Taxonomy as EntitiesTaxonomy;
    ...
    public function tags()
{
    return $this->belongsToMany(
        Tag::class,
        'lead_tags',   // 👈 your table
        'lead_id',
        'tag_id'
    )->withTimestamps()
     ->withPivot(['uuid','meta','created_by','updated_by','deleted_by','deleted_at']);
}
    public static function createItem($request)
    {
        ...
        if ($item) {
            $error_message = "This email already exist".($item->deleted_at?' in trash.':'.');
...
if (isset($inputs['tags']) && is_array($inputs['tags'])) {
    ...
    // ✅ extract only IDs
    $tag_ids = collect($inputs['tags'])->pluck('id')->toArray();
    ...
    {
        return $this->belongsTo(Taxonomy::class, 'status');
    }
...
public function followUp()
{
    ...
    public static function getList($request)
    {
        ...
        $user = auth()->user();
            ...
            if ($user && !$user->hasPermission('can-read-all-leads')) {
                $list->where('assigned_to', $user->id);
            }
        if($request->has('rows'))
        {
            $rows = $request->rows;
        ...
        if(isset($request->filter['status'])){
            $list->where('status', $request->filter['status']);
        }
     ...
     public static function listAction($request, $type): array
    {
        ...
        switch ($type) {
            ...
            if(!config('leadmanagement.is_dev')){
                $response['success'] = false;
    ...
    public static function getItem($id)
    {
        ...
        $item = self::where('id', $id)
            ->with(['assignedUser','createdByUser', 'updatedByUser', 'deletedByUser','tags'])
            ->withTrashed()
            ->first();
...
if (isset($inputs['tags']) && is_array($inputs['tags'])) {
    ...
    public static function validation($inputs)
    {
        ...
        if(isset($inputs['id'])){
            $rules['email'] = 'required|email|unique:leads,email,'.$inputs['id'];
        }
