...
let model_namespace = 'VaahCms\\Modules\\LeadManagement\\Models\\Lead';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/leadmanagement/leads";
    ...
    actions: {
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'leads.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'leads.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'leads.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'leads.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'leads.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
        ...
        async searchTags(event) {
            ...
            await vaah().ajax(
                ajax_url +'/tags' +'?search=' + query,
            this.searchTagsAfter
            );
            },
        
             searchTagsAfter(data,res){
                this.tags=data;
                
             },
             onTagSelect(e) {
                if (!this.query.filter.tag_ids) {
                    this.query.filter.tag_ids = [];
                }

                this.query.filter.tag_ids.push(e.value.id);
            },
