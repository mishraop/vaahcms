...
let model_namespace = 'VaahCms\\Modules\\LeadManagement\\Models\\Followup';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/leadmanagement/followups";
...
export const useFollowupStore = defineStore({
    id: 'followups',
    state: () => ({
        base_url: base_url,
        ajax_url: ajax_url,
    ...
    actions: {
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'followups.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'followups.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'followups.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'followups.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'followups.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
        ...
        async searchLeads(event) {
    let query = event.query;

    await vaah().ajax(
        ajax_url +'/lead' +'?search=' + query,
    this.searchLeadAfter
    );
},
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useFollowupStore, import.meta.hot))
}
